        ...
        const target = target: {
            if (rel.r_extern == 0) {
                ...
                const local_sym_index = context.object.sections_as_symbols.get(sect_id) orelse blk: {
                    ...
                    const use_stage1 = build_options.is_stage1 and context.macho_file.base.options.use_stage1;
                    const match = (try context.macho_file.getMatchingSection(sect, !use_stage1)) orelse
                        unreachable;
